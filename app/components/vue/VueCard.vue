...
<template>
	<NuxtLink :to="'/cases/' + item.slug.current" class="block w-full h-fit">
		<VueSquircle class="w-full h-fit md:relative group border border-black/12" :class="item.border ? '' : 'md:border-none'">
			<figure class="w-full h-full aspect-4/3">
				<SanityImage :asset-id="item.thumb.asset._ref" format="webp" class="w-full h-full object-cover group-hover:scale-102 transition duration-300" />
			</figure>
